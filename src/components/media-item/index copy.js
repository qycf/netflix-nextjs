...
export default function MediaItem({
  ...
  return (
    ...
    >
      ...
      <div className="relative  cardWrapper flex  h-28 min-w-[180px] cursor-pointer md:h-36 md:min-w-[260px] transform transition duration-500 hover:scale-[1.75] hover:z-[999]">
        <img
          src={`${media?.vodPic}`}
          alt="Media"
          layout="fill"
          className="rounded sm object-cover md:rounded hover:rounded-sm w-full h-auto"
          onClick={() =>
            router.push(
        ...
        />
        <div className=" flex-col hidden absolute p-2 bottom-0 buttonWrapper bg-black opacity-70">
          <h2 className="text-white text-lg font-bold">
            {media?.title || media?.vodName}
          </h2>
        ...
        {/* <button
            ...
            className={`${media?.addedToFavorites && !listView && "cursor-not-allowed"
              } cursor-pointer border flex p-2 items-center gap-x-2 rounded-full   font-semibold transition hover:opacity-90 border-white   bg-black opacity-75 text-black`}
          >
            {media?.addedToFavorites ? (
              <CheckIcon color="#ffffff" className="h-2 w-4" />
            ) : (
              <PlusIcon color="#ffffff" className="h-2 w-2" />
            )}
          </button>
          <button
            onClick={() => {
